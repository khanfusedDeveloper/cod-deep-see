...
static const int RX_BUF_SIZE = 1024; // We will set RX buffer to 0 for sender
...
void uart_init_sender(void)
{
    ...
    ESP_LOGI(TAG, "Sender UART initialized on port %d", UART_PORT);
}

static char hid_keycode_to_ascii(uint8_t keycode, bool shift)
{
    if (keycode >= 0x04 && keycode <= 0x1D)
    { // Letters a-z
        return (shift) ? (keycode - 0x04 + 'A') : (keycode - 0x04 + 'a');
    }
    ...
    { // Numbers 1-0
        if (shift)
        {
            ...
            return shifted_numbers[keycode - 0x1E];
        }
        else
        {
...
void hidh_callback(void *handler_args, esp_event_base_t base, int32_t id, void *event_data)
{
    ...
    {
        const uint8_t *bda = esp_hidh_dev_bda_get(param->input.dev);
        // ESP_LOGI(TAG, ESP_BD_ADDR_STR " INPUT: %s", ESP_BD_ADDR_HEX(bda), esp_hid_usage_str(param->input.usage));

        // We only want to parse data from a keyboard
        if (param->input.usage == ESP_HID_USAGE_KEYBOARD)
        {
            // A standard keyboard report is 8 bytes
            if (param->input.length == 8)
            {
                const uint8_t *p_data = param->input.data;
                // The first byte holds modifier keys (like Shift)
                bool shift_pressed = (p_data[0] & 0x22); // Checks for Left or Right Shift

                // The third byte holds the primary keycode
                uint8_t keycode = p_data[2];

                // A keycode > 0 means a key is being pressed down
                if (keycode > 0)
                {
                    // Translate the keycode to a readable character
                    char ascii_char = hid_keycode_to_ascii(keycode, shift_pressed);
                    if (ascii_char != '\0')
                    {
                        // Print the final character to the terminal
                        printf("KEY: %c\n", ascii_char);

                        uart_write_bytes(UART_PORT, &ascii_char, 1);
                    }
                }
            }
        }
